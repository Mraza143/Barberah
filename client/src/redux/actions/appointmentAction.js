...
export const createAppointmentForCustomers = (customerName, barberName, salonName, date, price) => async(dispatch) => {
    try {
        dispatch({ type: NEW_APPOINTMENT_REQUEST })
        const { data } = await axios.post("http://localhost:5000/api/appointments/create", customerName, barberName, salonName, date, price)
